...
def main():
    ...
    require("farGalaxies" in body and "farGalaxyStarsPerCluster" in body, "homepage galaxy must include sparse static distant background galaxies")
    require("universeBackdrop" in body and "universeStarCount" in body, "homepage galaxy must include a sparse static universe backdrop")
    ...
    require("radial-gradient" not in body, "homepage must not include decorative CSS circle backgrounds")
    require("linear-gradient(120deg" not in body, "homepage hero must not dim the galaxy with the old overlay backdrop")
    require("LineBasicMaterial" not in body and "new THREE.Line" not in body, "homepage galaxy must not render artificial line geometry")
